...
new Thread(() => {
    ...
    var trayIcon = new NotifyIcon()
    {
        ...
        ContextMenuStrip = new ContextMenuStrip()
        {
            Items =
            {
                new ToolStripMenuItem("queue", null, (_, _) => { new QueueForm().Show(); }, "queue"),
                new ToolStripMenuItem("close", null, (_, _) => { isRunning = false; }, "close")
            }
        },
        Visible = true
    };
...
internal class ReceiveCommandBehaviour : WebSocketBehavior
{
    ...
    public static readonly ReactField<Command?> Current = new();
...
internal class ReactQueue<T>: IReadOnlyCollection<T>, ICollection
{
    private readonly AutoResetEvent _ = new(true);
...
internal class QueueForm : Form
{
    ...
    private void UpdateUI()
    {
        ...
        label.Text = ReceiveCommandBehaviour.Current.Value != null ? $@"current: [{(ReceiveCommandBehaviour.Current.Value.ActionID == (int)Actions.Download ? "SteamCMD" : "download Cache")}] {ReceiveCommandBehaviour.Current.Value.ItemName}" : "";
